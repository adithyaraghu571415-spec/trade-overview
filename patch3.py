...
html = html.replace(
    ...
    '<div className="flex flex-wrap items-center gap-3 w-full sm:w-auto mt-4 sm:mt-0 max-w-full">'
)

# 3. Rework Customer Assignment Flow (Customer -> Brands)
panel_start = "<!-- ── CUSTOMER ASSIGNMENT PANEL ── -->"
panel_end = "<!-- ── MEDIA GALLERY ── -->"
...
if panel_start in html and panel_end in html:
    part1 = html.split(panel_start)[0]
    part2 = html.split(panel_end)[1]
    html = part1 + new_panel_jsx + "\n                " + panel_end + part2
...
dl_start = "const downloadCustomerPdfs = async () => {"
dl_end = "const saveEdits = async () => {"
...
new_capture_func = r"""const [custPdfData, setCustPdfData] = useState(null);
      ...
      """
if dl_start in html and dl_end in html:
    part1_dl = html.split(dl_start)[0]
    part2_dl = html.split(dl_end)[1]
    html = part1_dl + new_capture_func + dl_end + part2_dl
